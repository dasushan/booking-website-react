...
const Property = () => {
  ...
  const propertyInputRef = useRef(selectedProperty.document.name);
  const priceInputRef = useRef(selectedProperty.document.price);
  const addressInputRef = useRef(selectedProperty.document.address);
  const cityInputRef = useRef(selectedProperty.document.city);
  const pinInputRef = useRef(selectedProperty.document.pin);
  const propTypeInputRef = useRef(selectedProperty.document.type);
  const fileInputRef = useRef();
  // console.log(`${propertyInputRef.current} propertyInput`)
  ...
  useEffect(() => {
    setImageList(selectedProperty.document.images);
    setPreview(selectedProperty);
  }, []);
  ...
  const formSubmitHandler = (event) => {
    ...
    fetch(
      ...
      for (const [key, value] of Object.entries(result)) {
        if (value.id === selectedProperty.id) {
          keY = key;
        }
      }

      fetch(
        `https://travel-website-bc25b-default-rtdb.asia-southeast1.firebasedatabase.app/listings/${keY}.json`,
        {
          method: 'PUT',
          body: JSON.stringify({
            id: selectedProperty.id,
            document,
            booked: selectedProperty.booked,
          }),
        ...
        // console.log(result);
        // setPreview(result);
        fetch(`https://travel-website-bc25b-default-rtdb.asia-southeast1.firebasedatabase.app/listings/${keY}.json`, {
            method: 'GET',
        }).then(async (response) => {
            const result = await response.json();
            console.log(result);
  ...
  return (
    <div className="bg-slate-50 rounded-lg p-2 mx-2 flex-grow h-screen overflow-y-auto">
      ...
      <div className="text-sm text-slate-800 p-2 mx-3 ">
        <p className="text-xl font-semibold">Edit Listing</p>
      </div>
          ...
          <form className="p-2 m-1" onSubmit={formSubmitHandler}>
            <div className="mb-2">
              ...
              <input
                ...
                defaultValue={propertyInputRef.current}
            ...
            <div className="mb-2">
              ...
              <input
                ...
                id="price"
                className="border border-gray-200 w-full focus:outline-none cursor-pointer rounded-xl px-2 font-mono h-8 focus:shadow-sm"
                defaultValue={priceInputRef.current}
                ref={priceInputRef}
                required
              />
            </div>
            ...
            <div className="mb-2">
              ...
              <input
                ...
                id="city"
                className="border border-gray-200 w-full focus:outline-none cursor-pointer rounded-xl px-2 font-mono h-8 focus:shadow-sm"
                defaultValue={cityInputRef.current}
                ref={cityInputRef}
                required
              />
            </div>
            <div className="mb-2">
              ...
              <input
                ...
                id="pin"
                className="border border-gray-200 w-full focus:outline-none cursor-pointer rounded-xl px-2 font-mono h-8 focus:shadow-sm"
                defaultValue={pinInputRef.current}
                ref={pinInputRef}
                required
              />
            </div>
            <div className="mb-2">
              ...
              <select
                ...
                defaultValue={propTypeInputRef.current}
            ...
            <div className="flex items-center justify-end mb-2">
              ...
              {uploading && (
                <div className="ml-4 font-mono text-green-400">
                  Uploading...
                </div>
              )}
            </div>
              ...
              <CardHeader>
                ...
                <CardDescription>
                  <div>
                    ...
                    <div>
                      {preview.document.price} per night{' '}
                      <span>inculdes taxes and fees</span>
                    </div>
                  </div>
                </CardDescription>
              </CardHeader>
